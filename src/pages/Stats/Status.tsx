...
const Status: FC = () => {
    ...
    const { data: dataStat, error: errorStat, loading: loadingStat, getLogStat, resetData: resetStat } = useGetLogStreamStat();
    useEffect(() => {
            ...
            <Box className={headContainer}>
                ...
                <Box className={genterateContiner}>
                    ...
                    <Text className={genterateTextResult}>
                        {!loadingStat ? (errorStat ? "ERROR" : dataStat ? dayjs(dataStat?.time).format('DD-MM-YY HH:mm') : "") : "Loading..."}
                    </Text>
                </Box>
