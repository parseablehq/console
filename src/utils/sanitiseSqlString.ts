...
export const sanitiseSqlString = (sqlString: string, shouldNotify:boolean = true, limit: number = LOAD_LIMIT): string => {
    ...
    if (limitRegex.test(withoutTrailingSemicolon)) {
        return withoutTrailingSemicolon.replace(limitRegex, (match, p1) => {
            ...
            if (currentLimit > limit) {
                shouldNotify && notify({ message: `Limit exceeds the default load limit. Replacing with default limit - ${limit}` });
                return `LIMIT ${limit}`;
            }
            return match;
        });
